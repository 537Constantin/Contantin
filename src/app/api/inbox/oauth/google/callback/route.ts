...
import { googleConfigured, googleExchangeCode, googleCallbackUrl } from "@/lib/oauth";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** Google redirects here with ?code — we swap it for tokens and store them. */
export async function GET(req: NextRequest) {
  ...
  if (err) return fail(err === "access_denied" ? "Zugriff abgelehnt." : `Google-Fehler: ${err}`);
  ...
  if (!googleConfigured()) return fail("Google-Login ist nicht konfiguriert.");

  const scope = await resolveScope();
  if (!scope) return fail("Bitte zuerst anmelden.");

  const tokens = await googleExchangeCode(code, googleCallbackUrl(origin));
